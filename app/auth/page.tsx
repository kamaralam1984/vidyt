...
function AuthPageContent() {
    ...
    axios.get('/api/subscriptions/plans').then(res => {
      ...
      const payPlans = plans.filter((p: any) => p.price > 0).map((p: any) => {
        ...
        return {
          ...
          priceYear: Number(p.priceYearly) || (Number(p.price) * 10),
          period: p.interval || 'month',
          description: p.description || '',
          features: p.features || [],
          popular: p.popular || rawId === 'pro',
          discount: p.discount ? {
  ...
  const getPrice = (plan: Plan) => {
    ...
    let priceYearly = plan.priceYear;
    
    // Apply discount only for yearly in getPrice calculation
    if (billingPeriod === 'year' && plan.discount?.percentage) {
      priceYearly = priceYearly * (1 - plan.discount.percentage / 100);
    }
    ...
    if (!fxRates || locale.currency === 'USD') {
      return `$${format(usd)}${suffix}`;
    }
    const local = convertUsdToCurrency(usd, locale.currency, fxRates);
    return `${locale.currencySymbol}${format(local)}${suffix}`;
  };
                          ...
                          <ul className="space-y-2 max-h-48 overflow-y-auto pr-1 custom-scrollbar">
                            {plan.features.map((feature, idx) => (
                              <li key={idx} className="flex items-start gap-2 text-xs">
                                ...
                                <span className="text-[#AAAAAA] line-clamp-2">{feature}</span>
                              </li>
                            ))}
                          </ul>
                        </div>
